...
BATCH_SIZE = 128 # Number of samples per batch -- try smaller batch sizes -- otherwise, too long
NUM_EPOCH = 100 # Number of epochs (iterations over the entire dataset)
MIN_WORD_OCCURENCES = 5 # Minimum number of occurrences for a word to be included in the vocabulary
...
def save_embeddings_txt(data: GloveDataset, filename='glove_embeddings.txt'):
    ...
    with open(filename, 'w') as f:
        for idx, word in enumerate(data.indexer.index_to_word):
            vec = L_vecs[idx] + R_vecs[idx]  # optionally average the two embeddings -- originally, sum
...
def train_model(data: GloveDataset):
    ...
    for epoch in tqdm(range(NUM_EPOCH)):
        ...
        for batch_idx, batch in enumerate(tqdm(gen_batchs(data), total=num_batches, mininterval=1)):
            optimizer.zero_grad()
            loss = get_loss(*batch)
